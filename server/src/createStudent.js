...
async function run(){
  ...
  try{
    const [rows] = await conn.query('SELECT student_id FROM students WHERE email=? LIMIT 1',[email]);
    if(rows.length>0){
      console.log('Student already exists with email', email);
      return process.exit(0);
    }
    const hash = await bcrypt.hash(password,10);
    const [r] = await conn.query('INSERT INTO students (name,email,password_hash,roll_number,department) VALUES (?,?,?,?,?)',[name,email,hash,roll,dept]);
